...
export function isCorrect(question: Question, answer: string): boolean {
    let newAnswer: string = answer.toLowerCase().trim();

    if (question.expected.toLowerCase().trim()==newAnswer){
        return true;
    } else {
        return false;
    }
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === "multiple_choice_question"){
        return question.options.reduce((currentString: string, option: string) => currentString + "\n- " + option, result)
    }
    return result;
}
...
export function publishQuestion(question: Question): Question {
    if (question.published){
        ...
        return question = {...question, published:true};
    }
}
...
export function addOption(question: Question, newOption: string): Question {
    return question = {...question, options: [...question.options, newOption]};
}
